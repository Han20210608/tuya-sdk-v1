...
//Adafruit_NeoPixel strip(PIXEL_COUNT, PIXEL_PIN, NEO_GRB + NEO_KHZ800);  //2812灯珠线序
Adafruit_NeoPixel strip(PIXEL_COUNT, PIXEL_PIN, NEO_BRG + NEO_KHZ800);  //2811灯珠线序
...
SoftwareSerial mySerial(2,3); //使用软串口与涂鸦模块对接
...
unsigned char pid[] = {"8pyasf3dt4g9cnvt"};//*********处替换成涂鸦IoT平台自己创建的产品的PID
...
void loop() 
{
  ...
  if (digitalRead(key_pin) == LOW) 
  {
    ...
    if (digitalRead(key_pin) == LOW) 
    {
      ...
      Serial.println("connect network mode");
    }
  }
...
//          colorfill (strip.Color(  255, 255,  255));
    ...
    case DPID_BRIGHT_VALUE: //22 亮度
      bright_value_control(value, length);
      my_device.mcu_dp_update(dpid, value, length);
    ...
    case DPID_TEMP_VALUE: //23 冷暖值
      temp_value_control(value, length);
      my_device.mcu_dp_update(dpid, value, length); 
      
    break;

    case DPID_COLOUR_DATA:  //24 调色板
//    dp_string_value  = my_device.mcu_get_dp_download_data(dpid, value, length); /* Get the value of the down DP command */
    
//      Serial.println(DPID_COLOUR_DATA);
      my_device.mcu_dp_update(dpid, value, length);
      colour_data_control(value, length);
        
    break;
    
    case DPID_SCENE_DATA:  //25 情景
      scene_data_control(value, length);
...
//      colorfill (strip.Color( 255, 0,  0));
    ...
    case DPID_MUSIC_DATA: //27 音乐律动  
        music_data_control(value, length);

    break;
    
    case DPID_CONTROL_DATA: //28 实时调节
        control_data_control(value, length);
    break;
...
{
  my_device.mcu_dp_update(DPID_SWITCH_LED, dp_bool_value, 1); //20
  my_device.mcu_dp_update(DPID_WORK_MODE,  dp_enum_value, 1); //21
  my_device.mcu_dp_update(DPID_WORK_MODE,  dp_enum_value, 1); //21
  my_device.mcu_dp_update(DPID_WORK_MODE,  dp_enum_value, 1); //21
  my_device.mcu_dp_update(DPID_WORK_MODE,  dp_enum_value, 1); //21
...
void scene_data_control(const unsigned char value[], u16 length)  //25
{
//  u8 string_data[28];
  u16 h, s, v;
  u8 r, g, b;
...
//  string_data[0] = value[0];  //情景号
//  string_data[1] = value[1];  //
//  
//  string_data[2] = value[2];  //单元切换间隔时间（0-100）
//  string_data[3] = value[3];
//  
//  string_data[4] = value[4];  //单元变化时间（0-100）
//  string_data[5] = value[5];
//  
//  string_data[6] = value[6];  //单元变化模式（0静态1跳变2渐变）
//  string_data[7] = value[7];
//  
//  string_data[8] = value[8];  //hue（色度：0-360，0X0000-0X0168）
//  string_data[9] = value[9];
//  string_data[10] = value[10];
//  string_data[11] = value[11];
//  
//  string_data[12] = value[12];  //sat(饱和：0-1000, 0X0000-0X03E8)
//  string_data[13] = value[13];
//  string_data[14] = value[14];
//  string_data[15] = value[15];
//  
//  string_data[16] = value[16];  //val(明度：0-1000，0X0000-0X03E8)
//  string_data[17] = value[17]; 
//  string_data[18] = value[18]; 
//  string_data[19] = value[19]; 
//  
//  string_data[20] = value[20];  //白光亮度（0-1000）
//  string_data[21] = value[21]; 
//  string_data[22] = value[22]; 
//  string_data[23] = value[23]; 
//  
//  string_data[24] = value[24];  //色温值（0-1000）
//  string_data[25] = value[25]; 
//  string_data[26] = value[26]; 
//  string_data[27] = value[27]; 
  

  bright = __str2short(__asc2hex(value[20]), __asc2hex(value[21]), __asc2hex(value[22]), __asc2hex(value[23]));
  ...
  if(bright == 0)
  {
    h = __str2short(__asc2hex(value[8]), __asc2hex(value[9]), __asc2hex(value[10]), __asc2hex(value[11]));
    s = __str2short(__asc2hex(value[12]), __asc2hex(value[13]), __asc2hex(value[14]), __asc2hex(value[15]));
    v = __str2short(__asc2hex(value[16]), __asc2hex(value[17]), __asc2hex(value[18]), __asc2hex(value[19]));
    hue=h*182;
    sat=s/4;
    val=v/4;
...
//    Serial.println(val);
  ...
  {
    s = __str2short(__asc2hex(value[24]), __asc2hex(value[25]), __asc2hex(value[26]), __asc2hex(value[27]));
    v = __str2short(__asc2hex(value[20]), __asc2hex(value[21]), __asc2hex(value[22]), __asc2hex(value[23]));
    sat=250-s/4;
    val=v/4;
    
    c = strip.gamma32(strip.ColorHSV(9000,sat,val)); // hue -> RGB
//      Serial.println(hue);
//      Serial.println(sat);
//      Serial.println(val);
...
void bright_value_control( const unsigned char value[], u16 length)
{
//    u8 string_data[4];
    u16 h, s, v;
    u8 r, g, b;
...
//    string_data[3] = value[3];
    ...
    //这里的最大值为0x03E8 直接就是十六进制数，不用转换
    // HSV 中的 S 代表饱和度，H固定 后 V调节亮度
//    s = __str2short(string_data[0],string_data[1],string_data[2],string_data[3]);
    v = value[2] * 256 + value[3];
...
void temp_value_control( const unsigned char value[], u16 length)
{
//    u8 string_data[4];
    u16 h, s, v;
    u8 r, g, b;
...
void music_data_control( const unsigned char value[], u16 length)
{
//  u8 string_data[21];
  u16 h, s, v,temp;
  u8 r, g, b;
...
//  
//  string_data[1] = value[1];//hue
//  string_data[2] = value[2];
//  string_data[3] = value[3];
//  string_data[4] = value[4];
//  
//  string_data[5] = value[5];//sat
//  string_data[6] = value[6];
//  string_data[7] = value[7];
//  string_data[8] = value[8];
//  
//  string_data[9] = value[9];//val
//  string_data[10] = value[10];
//  string_data[11] = value[11];
//  string_data[12] = value[12];
//  
//  string_data[13] = value[13];//bright
//  string_data[14] = value[14];
//  string_data[15] = value[15];
//  string_data[16] = value[16];
//  
//  string_data[17] = value[17];//色温
//  string_data[18] = value[18];
//  string_data[19] = value[19];
//  string_data[20] = value[20];
...
void control_data_control( const unsigned char value[], u16 length)
{
//  u8 string_data[21];
  u16 h, s, v,temp;
  u8 r, g, b;
...
//          colorfill (strip.Color(  255, 255,  255));
      ...
      temp = __str2short(__asc2hex(value[17]), __asc2hex(value[18]), __asc2hex(value[19]), __asc2hex(value[20]));  
...
//        Serial.println(temp);
      ...
      if((temp == 0)&&(value[16] != 0x61)) //判断为长按亮度
      {
        hue = 9000;
        sat = Sat;
        v = __str2short(__asc2hex(value[13]), __asc2hex(value[14]), __asc2hex(value[15]), __asc2hex(value[16]));
        val = v/4;
        Val = val;
...
//  else
//  {
//    c = strip.gamma32(strip.ColorHSV(9000,Sat,bright)); // hue -> RGB
//  //      Serial.println(hue);
//  //      Serial.println(sat);
//  //      Serial.println(val);
//    Serial.println(bright);
...
//    string_data[0] = value[0];//h
//    string_data[1] = value[1];
//    string_data[2] = value[2];
//    string_data[3] = value[3];
//    
//    string_data[4] = value[4];//s
//    string_data[5] = value[5];
//    string_data[6] = value[6];
//    string_data[7] = value[7];
//    
//    string_data[8] = value[8];//v
//    string_data[9] = value[9];
//    string_data[10] = value[10];
//    string_data[11] = value[11];
    
  
    h = __str2short(__asc2hex(value[0]), __asc2hex(value[1]), __asc2hex(value[2]), __asc2hex(value[3]));
    s = __str2short(__asc2hex(value[4]), __asc2hex(value[5]), __asc2hex(value[6]), __asc2hex(value[7]));
    v = __str2short(__asc2hex(value[8]), __asc2hex(value[9]), __asc2hex(value[10]), __asc2hex(value[11]));
